...
class StructuredOutputRequest:
    ...
    def _bind_grammar_to_request(self):
        ...
        if isinstance(self._grammar, StructuredOutputGrammar) and self.request_id:
    ...
    def _check_grammar_completion(self) -> bool:
        ...
        if isinstance(self._grammar, Future):
            try:
                ...
                # ✅ 追加：若开启审计，提前建立上下文（不阻塞）
                try:
                    if isinstance(self._grammar, StructuredOutputGrammar):
                        # 某些场景下请求ID属性名不同，这里做个容错获取
                        rid = getattr(self, "request_id", None) or getattr(self, "id", None)
                        if rid:
                            self._grammar.set_audit_context(rid)
                except Exception:
                    # 静默降级，不影响主流程
                    pass
            except TimeoutError:
