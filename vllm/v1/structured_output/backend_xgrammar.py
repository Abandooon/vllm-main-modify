...
class XgrammarGrammar(StructuredOutputGrammar):
    ...
    _termination_logged: bool = field(default=False, repr=False, hash=False)  # ✅ 新增

    def _get_current_state_id(self) -> str:
        """尽力获取可读的状态ID；若后端不暴露则回退到可追踪信息。"""
        # 尝试调试接口（若 xgrammar 暴露）
    ...
    def accept_tokens(self, request_id: str, tokens: list[int]) -> bool:
        ...
        for token in tokens:
            if not self.matcher.accept_token(token):
                # 失败路径：记录 TOKEN_REJECT，并保持原有返回逻辑
    ...
    def is_terminated(self) -> bool:
        # 仅返回状态，但在 accept_tokens 里已处理首次进入终止态的打点
    ...
    def reset(self):
        ...
        self._termination_logged = False  # 避免复用旧标志
